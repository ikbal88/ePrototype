                  ...
                  <div id="detail">
                    <div class="detailLine form-group">
                      <div class="itemLine col-md-4">
                        <input class="form-control" name="id_barang[]" placeholder="Scan Barcode" type="text" value="">
                        <span class="help-block with-errors"></span>
                      </div>
...
function viewData(id) {
  $.ajax({
    ...
    success: function(data) {
      ...
      var total_trans = ribuan(d[0].total_trans);
      for (var i = 0; i < d.length; i++) {
        ...
        <td>'+d[i].id_barang+'</td>\n\
        <td>'+catatan+'</td>\n\
        <td>'+ribuan(d[i].price)+'</td>\n\
        <td>'+ribuan(d[i].total)+'</td>\n\
        </tr>';
      }
      s += '<tr>\n\
...
function printData(id) {
  window.open(baseurl+'/maklon/'+id+'/print');
}
